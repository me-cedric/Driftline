...
enum BcryptPBKDF {
    static func derive(password: Data, salt: Data, outputLength: Int, rounds: UInt32) throws -> Data {
        guard outputLength > 0 else { return Data() }
        guard rounds > 0 else { throw BcryptPBKDFError.invalidRounds }

        let sha2pass = Data(SHA512.hash(data: password))
        let stride = (outputLength + 31) / 32
        var amount = (outputLength + stride - 1) / stride
        var remainingOutputLength = outputLength
        var result = Data(repeating: 0, count: outputLength)

        for count in 1 ... stride where remainingOutputLength > 0 {
            var countsalt = salt
            ...
            let sha2salt = Data(SHA512.hash(data: countsalt))
            var tmpout = bcryptHash(sha2pass: sha2pass, sha2salt: sha2salt)
            var out = tmpout

            for _ in 1 ..< rounds {
                let nextSalt = Data(SHA512.hash(data: tmpout))
                tmpout = bcryptHash(sha2pass: sha2pass, sha2salt: nextSalt)
                for i in 0 ..< 32 {
                    out[i] ^= tmpout[i]
                }
            }

            amount = min(amount, remainingOutputLength)
            var written = 0
            for i in 0 ..< amount {
                let destination = i * stride + (count - 1)
                guard destination < outputLength else { break }
                result[destination] = out[i]
                written += 1
            }
            remainingOutputLength -= written
        }

        return result
    }

    private static func bcryptHash(sha2pass: Data, sha2salt: Data) -> Data {
        ...
        var words: [UInt32] = (0 ..< 8).map { i in
            let base = i * 4
            return (UInt32(magic[base]) << 24) |
                (UInt32(magic[base + 1]) << 16) |
                (UInt32(magic[base + 2]) << 8) |
                UInt32(magic[base + 3])
        }

        for _ in 0 ..< 64 {
            for pair in 0 ..< 4 {
                let (l, r) = state.encrypt(l: words[pair * 2], r: words[pair * 2 + 1])
                words[pair * 2] = l
                words[pair * 2 + 1] = r
            }
        }
        ...
        for i in 0 ..< 8 {
            output[i * 4 + 0] = UInt8(words[i] & 0xff)
            output[i * 4 + 1] = UInt8((words[i] >> 8) & 0xff)
            output[i * 4 + 2] = UInt8((words[i] >> 16) & 0xff)
            output[i * 4 + 3] = UInt8((words[i] >> 24) & 0xff)
        }
        return output
    }
}

enum BcryptPBKDFError: Error {
    case invalidRounds
}
...
private struct BlowfishState {
    ...
    init() {
        self.p = BlowfishConstants.p
        self.s0 = BlowfishConstants.s0
        self.s1 = BlowfishConstants.s1
        self.s2 = BlowfishConstants.s2
        self.s3 = BlowfishConstants.s3
    }

    mutating func encrypt(l: UInt32, r: UInt32) -> (UInt32, UInt32) {
        var xl = l ^ self.p[0]
        var xr = r
        ...
        return (xr ^ self.p[17], xl)
    }

    private func f(_ x: UInt32) -> UInt32 {
        let a = self.s0[Int((x >> 24) & 0xff)]
        let b = self.s1[Int((x >> 16) & 0xff)]
        let c = self.s2[Int((x >> 8) & 0xff)]
        let d = self.s3[Int(x & 0xff)]
        return ((a &+ b) ^ c) &+ d
    }
    ...
    mutating func expandState(salt: Data, key: Data) {
        ...
        for i in 0 ..< 18 {
            var word: UInt32 = 0
            for _ in 0 ..< 4 {
                word = (word << 8) | UInt32(key[keyPos % keyLen])
                keyPos += 1
            }
            self.p[i] ^= word
        }
        ...
        for i in stride(from: 0, to: 18, by: 2) {
            l ^= self.streamWord(data: salt, pos: &saltPos, len: saltLen)
            r ^= self.streamWord(data: salt, pos: &saltPos, len: saltLen)
            let (nl, nr) = self.encrypt(l: l, r: r)
            l = nl
            r = nr
            self.p[i] = l
            self.p[i + 1] = r
    ...
    private func streamWord(data: Data, pos: inout Int, len: Int) -> UInt32 {
        var word: UInt32 = 0
        for _ in 0 ..< 4 {
            word = (word << 8) | UInt32(data[pos % len])
            pos += 1
        }
        return word
    }
